...
class Lidar_subscriber(Node):
    ...
    def lidar_callback(self, msg):
        ...
        # Replace inf and nan with max range (e.g., 10.0 or 12.0)
        raw_ranges[np.isinf(raw_ranges)] = 10.0
        raw_ranges[np.isnan(raw_ranges)] = 10.0
        
        # Focus on the front 180 degrees (90 to 270 degrees in a 360 scan)
        # Sample exactly 20 points as required by your network
        indices = np.linspace(90, 270, 20).astype(int)
        lidar_data = raw_ranges[indices]
...
if __name__ == '__main__':
    ...
    file_name = "td3_green_4wheel" # name of the file to load the policy from
    environment_dim = 20
    robot_dim = 4
    ...
    try:
        network.load(file_name, "/home/shesha/robot_ws/src/td3/scripts/models") # Note the folder change too
    except:
